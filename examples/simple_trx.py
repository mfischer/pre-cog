...
# Title: Simple Trx
# Generated: Thu Oct  4 14:52:01 2012
##################################################
...
if __name__ == '__main__':
	...
	tb = simple_trx(radio_addr=options.radio_addr, rate=options.rate, tx_gain=options.tx_gain, samp_per_sym=options.samp_per_sym, freq=options.freq, dest_addr=options.dest_addr, arq_timeout=options.arq_timeout, max_arq_attempts=options.max_arq_attempts, rx_freq=options.rx_freq, rx_gain=options.rx_gain, args=options.args, ampl=options.ampl, port=options.port)
	tb.Run(True)
